...
export default class ItemsBlock extends Component {
	...
	render() {
		...
		//console.log(":::::: "+this.props.itemBlockIndex)
		...
		if (this.props.items.length > 0) {
			const startIndex =  this.props.itemBlockIndex * this.props.numItemsToDisplayAtATime
						...
						{fourItems.length === 4 &&
							<figure>
								<ImgShell>
									...
									<RateButton onClick={()=>this.props.onItemFavorited(fourItems[i].id)}>
										<MaterialIcon icon="favorite" size={30} color={colors.white} />
									</RateButton>
									{ fourItems[i].type==='artist' &&
										<PlayIcon>
											<MaterialIcon icon="play_circle_outline" size={55} color={colors.white} />
										</PlayIcon>
									}
								</ImgShell>
								<figcaption>
